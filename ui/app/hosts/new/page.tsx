...
export default function NewPage() {
  ...
  async function createHost(data) {
    const res = await Post("/hosts", data);
  ...
  return (
    <main className="p-4 md:p-10 mx-auto max-w-7xl">
      <Card className="mt-6 mb-5">
        <div>
          <Formik
            initialValues={{ name: '', address: '', port: '', username: '', password: '' }}
            validate={values => {
              const errors = {};
              if (!values.name) {
            ...
            onSubmit={(values, { setSubmitting }) => {
              setTimeout(() => {
                ...
                createHost(JSON.stringify([values]));
                setSubmitting(false);
              }, 400);
            }}
          >
            {({ isSubmitting }) => (
              <Form>
                ...
                <button disabled={isSubmitting} type="submit" className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Create host</button>
